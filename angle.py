...
def find_angle_from_df(filename,pt1,pt2,pt3):
    df = pd.read_csv(filename + '.csv')
    for i in range(len(df)):
        x1 = (df.loc[i]['x'+str(pt1)])
        y1 = (df.loc[i]['y'+str(pt1)])
        x2 = (df.loc[i]['x'+str(pt2)])
        y2 = (df.loc[i]['y'+str(pt2)])
        x3 = (df.loc[i]['x'+str(pt3)])
        y3 = (df.loc[i]['y'+str(pt3)])
